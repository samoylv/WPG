...
#    II = (np.abs(Uc)**2).sum(axis=0) / repeats  # intensity by definition
    ...
    lm = lm[(axisEx[lm] > 0) & (Jd[lm] < 0.5)] ###CHANGE HERE
    if len(lm) > 0:
    ...
    *locLegend*: str, legend location in matplotlib style.
    ...
    if fig2 is None:
        ...
        if isIntensityNormalized:
            ...
            ax2.set_ylabel("Electric Field Amplitude (a.u.)".format(unit),
                           color="C0")
    ...
    legendX = ax3.legend(
        (l1, l3, pp),
        ("I({0}),\nσ$_{0}$ = {1:.2f} $\mu$m".format(axisName, varIx**0.5*1e3),
         "DoC({0}$_1-${0}$_2$),\nξ$_{0}$ = {1:.2f} $\mu$m".format(
             axisName, varJdx**0.5*1e3),
